...
SELECT customer.customer_id, city, country, quantity, shipping.shipment_status, price, brand, subcategory.subcategory_name, review_score, category.category_name, product_name, product.product_id, promo_price, order_date
FROM customer
INNER JOIN address
...
LEFT JOIN transaction_billing
ON \"order\".order_id = transaction_billing.order_id
LEFT JOIN shipping
ON shipping.billing_id = transaction_billing.billing_id
LEFT JOIN subcategory
ON subcategory.category_id = category.category_id
LEFT JOIN review
ON review.customer_id = customer.customer_id
LEFT JOIN promotion
ON promotion.category_id = category.category_id
")

## Seller by product category
...
ggplot(category_sales, aes(x = sales_amount, 
...
# heatmap by cities
sales_data %>% group_by(country, city) %>% summarise(sales_amount = sum(sales_amount)) %>% 
...
sales_data %>% group_by(country) %>% summarise(Total_sales_amount = sum(sales_amount)) %>% 
...
# sales trend
ggplot(sales_by_date, aes(x = order_date, y = sales_amount)) + geom_line() +
