...
```{sql, connection = my_db}
INSERT INTO category (category_id, category_name, sub_category, category_description)
SELECT category_id, category_name, sub_category, category_description FROM category_data;
...
# 15/03 DB table modification
```{sql, connection = my_db}
-- add "card_number" in customer table
ALTER TABLE customer
ADD COLUMN card_number INT(16);
```

```{sql, connection = my_db}
-- add category info in product table
ALTER TABLE product
ADD COLUMN category_name VARCHAR(50)
ADD COLUMN subcategory_name VARCHAR(50);
```
```{sql, connection = my_db}
-- add sub category info in product table
ALTER TABLE product
ADD COLUMN subcategory_name VARCHAR(50);
```
